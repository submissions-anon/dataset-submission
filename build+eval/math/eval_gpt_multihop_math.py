"""
eval_gpt_multihop_math.py
=========================
GPT-5.2 + Gemini 4-hop retrieval system for Math Analogues benchmark.

Task: Given a math problem, find other problems requiring the same core reasoning pattern.

Pipeline per query:
  For each hop (1-4):
    1. GPT-5.2 generates a search query from (original_query + accumulated_notes)
    2. Gemini retrieves top-K problems via dense embedding similarity
    3. GPT-5.2 reads retrieved problems, selects pertinent ones + extracts notes
...
Usage:
  ...
  python eval_gpt_multihop_math.py
  python eval_gpt_multihop_math.py --limit 5  # smoke test
"""
...
parser.add_argument("--top-k-per-hop", type=int, default=25,
                    help="Problems to retrieve per hop")
...
parser.add_argument("--ckpt", default="checkpoints/gpt_multihop_math_cache.jsonl")
...
parser.add_argument("--max-doc-chars", type=int, default=99999,
                    help="Max chars of problem text shown to GPT per document")
...
QUERY_GEN_SYSTEM = """\
You are an expert at finding mathematical problems that share the same core reasoning pattern.

Given an original math problem and notes from previous search iterations, generate a search
query that will help find other problems requiring the same "aha moment" or eureka insight.

The search query should:
1. Focus on the underlying technique or reasoning pattern, not surface-level topic
2. Use mathematical terminology that would appear in similar problems
3. Be different from previous search angles to maximize coverage

If this is the first hop, focus on the core reasoning pattern. For later hops, refine based
on what analogous structures you've discovered."""

QUERY_GEN_USER = """\
ORIGINAL PROBLEM (find analogues with same reasoning pattern):
{original_query}
...
NOTE_EXTRACT_SYSTEM = """\
You are an expert at identifying mathematical problems that share the same core reasoning pattern.

Your tasks:
1. Select problems that require the SAME "aha moment" or eureka insight as the query problem
2. Write brief notes about the reasoning patterns you observed

A problem is an analogue ONLY IF it requires the same fundamental technique or insight,
even if the surface-level topic is completely different."""

NOTE_EXTRACT_USER = """\
QUERY PROBLEM (find analogues):
{query}

PREVIOUS NOTES:
{previous_notes}

CANDIDATE PROBLEMS (from hop {hop_num}):
{candidates}

Analyze these problems. Return ONLY a JSON object:
{{
  "candidate_ids": ["id1", "id2", ...],
  "notes": "<observations about reasoning patterns, what types of analogues are MISSING>",
  "summary": "<brief summary of what was found in this hop>"
}}

{selection_instruction}
Add notes that could help find more analogous problems in the next hop."""

RERANK_SYSTEM = """\
You are an expert at identifying mathematical problems that share the same core reasoning pattern.

Rank the candidates from most to least analogous to the query problem.
Focus on the underlying "aha moment" or technique, not surface-level similarity.
...
Rank all {k} candidates by how closely they share the same reasoning pattern."""
...
def run_multihop_pipeline(query_id, query_text, corpus_by_id, corpus_embeddings, corpus_ids, excluded_ids=None):
    """
    ...
    hop_details = []
    # Initialize seen_docs with excluded IDs (e.g., the query problem itself)
    ...
    for hop_num in range(1, args.num_hops + 1):
        ...
        print(f"      Retrieved {len(retrieved)} problems")
        ...
        if args.max_candidates_per_hop:
            selection_instruction = f"Select up to {args.max_candidates_per_hop} candidate IDs that share the same reasoning pattern."
        else:
            selection_instruction = "Select ALL candidate IDs that share the same reasoning pattern (no limit)."
...
print(f"\n{'='*70}")
print(f"  GPT-5.2 + Gemini {args.num_hops}-Hop Retrieval System (Math Analogues)")
...
corpus_by_id = {}
corpus_ids = []
for doc in tqdm(iter_corpus(corpus_file), desc="  Loading", unit="problem"):
    corpus_by_id[doc["_id"]] = {"text": doc["text"]}
    corpus_ids.append(doc["_id"])
print(f"  {len(corpus_ids):,} problems")
...
# Load per-query excluded IDs (e.g., the query problem itself shouldn't be retrieved)
per_query_excluded = {}
excluded_ids_file = dataset_dir / "per_query_excluded_ids.json"
...
corpus_emb_path = ckpt_dir / f"corpus_{model_slug}_framed.npy"
...
if corpus_emb_path.exists():
    ...
    raise FileNotFoundError(
        ...
        f"Run eval_math_analogues.py first to generate them."
    )
...
for qid in tqdm(query_ids, desc="Processing queries"):
    ...
    # Get excluded IDs for this query (don't retrieve the query problem itself)
...
print(f"\n{'='*70}")
print(f"  {args.model} + Gemini  —  {args.num_hops}-hop retrieval (Math Analogues)")
print(f"  Top-K/hop: {args.top_k_per_hop}  |  Max candidates/hop: {args.max_candidates_per_hop}")
print(f"  Corpus: {len(corpus_ids):,} problems  |  Queries: {len(query_ids)}")
print(f"{'='*70}")
...
print(f"\n{'='*70}")
print(f"  SUMMARY: {args.model} + Gemini {args.num_hops}-hop (Math Analogues)")
print(f"{'='*70}")
