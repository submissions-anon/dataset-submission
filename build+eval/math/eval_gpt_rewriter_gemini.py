...
===========================
GPT-5.2 query rewriter + Gemini dense retrieval for Math Analogues benchmark.
...
parser.add_argument("--ckpt", default="checkpoints/gpt_rewriter_gemini_math_cache.jsonl")
...
Given a math problem, rewrite the query to better capture the underlying technique or "aha moment"
that would help find other problems requiring the same insight. Focus on the mathematical structure
and reasoning pattern, not surface-level topic.
...
Rewrite this to capture the core reasoning pattern for finding analogous problems."""
...
print(f"\n{'='*70}")
print(f"  GPT-5.2 Query Rewriter + Gemini Retrieval (Math Analogues)")
...
if corpus_emb_path.exists():
    ...
    raise FileNotFoundError(
        ...
        f"Run the base Gemini eval first to generate them."
    )
...
for qid in tqdm(query_ids, desc="Processing"):
    ...
    retrieved = retrieve_top_k(
        query_emb[0], corpus_embeddings, corpus_ids, args.top_k,
    ...
    rec = {
        ...
        "rationale": rationale,
        "top_results": retrieved,  # Save all top-K results
    }
    results_list.append(rec)
...
print(f"\n{'='*70}")
print(f"  {args.model} Rewriter + Gemini  (Math Analogues)")
...
print(f"\n  {'qid':<10} {'n_rel':>5}  {'NDCG@10':>8}  {'R@10':>7}  {'R@50':>7}  {'R@100':>7}")
print(f"  {'-'*55}")
...
print(f"\n{'='*70}")
print(f"  SUMMARY: {args.model} Rewriter + Gemini (Math Analogues)")
print(f"{'='*70}")
