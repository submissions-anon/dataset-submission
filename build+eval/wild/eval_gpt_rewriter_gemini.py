...
===========================
GPT-5.2 query rewriter + Gemini dense retrieval for WildChat benchmark.
...
Usage:
  ...
  python eval_gpt_rewriter_gemini.py --dataset-dir dataset/merged
  python eval_gpt_rewriter_gemini.py --dataset-dir dataset/merged --limit 5  # smoke test
"""
...
parser.add_argument("--ckpt", default="checkpoints/gpt_rewriter_gemini_wildchat_abtract_cache.jsonl")
...
REWRITE_SYSTEM = """\
You are an expert at reformulating queries to improve retrieval of relevant LLM conversations.

Given a query describing an LLM failure pattern or behavior, rewrite it to better match how
such failures would manifest in actual conversations. Focus on the language, phrasing, and
patterns that would appear in relevant LLM interactions.
...
Rewrite this query to improve retrieval of LLM conversations exhibiting this pattern."""
...
print(f"\n{'='*70}")
print(f"  GPT-5.2 Query Rewriter + Gemini Retrieval (WildChat)")
...
print(f"  {len(corpus_ids):,} conversations")
...
for qid in tqdm(query_ids, desc="Processing"):
    ...
    rec = {
        ...
        "rationale": rationale,
        "top_results": retrieved,  # Save top 100 for inspection
    }
    results_list.append(rec)
...
print(f"\n{'='*70}")
print(f"  {args.model} Rewriter + Gemini  (WildChat)")
...
print(f"\n{'='*70}")
print(f"  SUMMARY: {args.model} Rewriter + Gemini (WildChat)")
print(f"{'='*70}")
