"""
eval_gpt_multihop_wildchat.py
=============================
GPT-5.2 + Gemini 4-hop retrieval system for WildChat Descriptive-IR benchmark.

Pipeline per query:
  For each hop (1-4):
    1. GPT-5.2 generates a search query from (original_query + accumulated_notes)
    2. Gemini retrieves top-K documents via dense embedding similarity
    3. GPT-5.2 reads retrieved docs, selects pertinent ones + extracts notes
...
Usage:
  ...
  python eval_gpt_multihop_wildchat.py --dataset-dir dataset/merged
  python eval_gpt_multihop_wildchat.py --dataset-dir dataset/merged --limit 5  # smoke test
"""
...
parser.add_argument("--top-k-per-hop", type=int, default=25,
                    help="Documents to retrieve per hop")
...
parser.add_argument("--ckpt", default="checkpoints/gpt_multihop_wildchat_cache.jsonl")
...
parser.add_argument("--max-doc-chars", type=int, default=99999,
                    help="Max chars of conversation text shown to GPT per document")
...
QUERY_GEN_SYSTEM = """\
You are an expert at iterative information retrieval for finding LLM failure patterns.

Given an original query describing an LLM failure mode, and notes from previous search
iterations, generate a focused search query that will help find more relevant conversations.

The search query should:
1. Target specific aspects of the failure pattern not yet well-covered
2. Use concrete keywords likely to appear in relevant conversations
...
core failure pattern. For later hops, refine based on what's been learned."""

QUERY_GEN_USER = """\
ORIGINAL QUERY (failure pattern to find):
{original_query}
...
NOTE_EXTRACT_SYSTEM = """\
You are an expert at analyzing LLM conversations for failure patterns.

Your tasks:
1. Select conversations that exhibit the failure pattern described in the query
2. Write brief notes about what you observed (to inform the next search)

A conversation is relevant ONLY IF the AI's response violates the constraint in the way the query describes."""
...
CANDIDATE CONVERSATIONS (from hop {hop_num}):
{candidates}

Analyze these conversations. Return ONLY a JSON object:
{{
...
Add notes that could help find more relevant conversations in the next hop."""

RERANK_SYSTEM = """\
You are an expert at identifying LLM failure modes in human-AI conversations.

Rank the candidates from most to least relevant to the failure pattern in the query.
...
def run_multihop_pipeline(query_id, query_text, corpus_by_id, corpus_embeddings, corpus_ids):
    ...
    for hop_num in range(1, args.num_hops + 1):
        ...
        if args.max_candidates_per_hop:
            selection_instruction = f"Select up to {args.max_candidates_per_hop} candidate IDs that best match the failure pattern."
        else:
            selection_instruction = "Select ALL candidate IDs that match the failure pattern (no limit)."
...
print(f"\n{'='*70}")
print(f"  GPT-5.2 + Gemini {args.num_hops}-Hop Retrieval System")
...
corpus_emb_path = ckpt_dir / f"corpus_wildchat_{model_slug}.npy"
...
if corpus_emb_path.exists():
    ...
    raise FileNotFoundError(
        ...
        f"Run eval_wildchat_gemini.py first to generate them."
    )
...
print(f"\n{'='*70}")
print(f"  {args.model} + Gemini  —  {args.num_hops}-hop retrieval")
...
print(f"\n{'='*70}")
print(f"  SUMMARY: {args.model} + Gemini {args.num_hops}-hop")
print(f"{'='*70}")
